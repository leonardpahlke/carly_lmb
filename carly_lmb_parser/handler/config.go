	...
	for {
		...
		case html.StartTagToken, html.SelfClosingTagToken:
			...
			for _, textTag := range request.TextTagsToParse {
				if tag == textTag && !pkg.CheckElementAttribute(request.WhitelistAttributeValues, tagAttribute) {
					enter = true
					break
				}
			}
...
func getElement(doc *html.Node, element string, requiredAttribute []html.Attribute) (*html.Node, error) {
	...
	crawler = func(node *html.Node) {
		if node.Type == html.ElementNode && node.Data == element && pkg.CheckElementAttribute(requiredAttribute, node.Attr) {
			elementNode = node
			return
		}
