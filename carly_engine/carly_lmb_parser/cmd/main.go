...
import (
	lmb_parser "carly_lmb_parser/handler"
	"fmt"
	"io/ioutil"
	"math/rand"
	"net/http"
...
var name = "LocalTest-" + pkg.CARLY_ENGINE_LMB_PARSER.Name

func main() {
	_ = os.Setenv("AWS_REGION", pkg.AWSDeployRegion)
	_ = os.Setenv(pkg.EnvSpiderName, pkg.CARLY_ENGINE_LMB_PARSER.Name)
...
func testNewspaper(newspaper string, articleUrls []string, forceWriteArticleText bool, writeArticleDom bool, folderPath string) {
	for _, articleUrl := range articleUrls {
		...
		ok := reqSpiderParser(pkg.CarlyEngineLmbParserEvent{
...
func reqSpiderParser(event pkg.CarlyEngineLmbParserEvent, forceStore bool, folderPath string) bool {
	resp, err := lmb_parser.Handler(event)
	if err != nil {
